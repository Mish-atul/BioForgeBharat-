...
const GlassCard = ({ children, className }: { children: React.ReactNode, className?: string }) => (
  <div className={cn("p-1.5 rounded-[2.5rem] bg-white/[0.04] border border-white/10 shadow-[0_0_40px_rgba(0,0,0,0.5)]", className)}>
    ...
    <div className="space-y-10 animate-in fade-in duration-500 pb-20">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="flex justify-between items-end gap-6 flex-wrap">
        <div>
          ...
          <h1 className="text-5xl font-serif font-black tracking-tight text-white mb-2 drop-shadow-lg">Model Retraining</h1>
          <p className="text-lg text-white/60 max-w-xl font-medium">Closed-loop learning from experimental feedback with provenance.</p>
        </div>
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger asChild>
            <Button className="gap-2 bg-gradient-to-r from-cyan-600 to-blue-500 text-white rounded-full px-8 py-6 font-bold shadow-[0_0_20px_rgba(6,182,212,0.4)] hover:shadow-[0_0_30px_rgba(6,182,212,0.6)] transition-all">
              <Settings2 className="w-5 h-5" />
              Trigger Retraining
            </Button>
          </DialogTrigger>
          <DialogContent className="sm:max-w-[450px] bg-[#0F0C29]/95 backdrop-blur-3xl border-white/20 text-white rounded-[2rem] shadow-2xl">
            <DialogHeader>
              <DialogTitle className="text-3xl font-serif font-bold text-white mb-2">Trigger Retraining</DialogTitle>
            </DialogHeader>
            <Form {...form}>
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                <p className="text-sm font-medium text-white/50 leading-relaxed bg-black/30 p-4 rounded-xl border border-white/5">
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel className="text-white/70">Authorized Supervisor</FormLabel>
                      <FormControl>
                        <Input className="bg-black/40 border-white/10 focus-visible:ring-cyan-500 rounded-xl px-4 py-6 text-lg" placeholder="Dr. Arjun Patel" {...field} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
                ...
                <div className="flex justify-end pt-4">
                  <Button type="submit" disabled={triggerRun.isPending} className="w-full bg-cyan-600 hover:bg-cyan-500 text-white font-bold rounded-full py-6 text-lg">
        ...
        <motion.div variants={staggerContainer as any} initial="hidden" animate="visible" className="grid grid-cols-1 sm:grid-cols-3 gap-6">
          <motion.div variants={staggerChild as any}>
            <GlassCard className="hover:scale-[1.02] transition-transform">
              <div className="text-[10px] text-white/40 font-bold uppercase tracking-widest mb-4">Current Accuracy</div>
              <div className="text-5xl font-mono font-black text-cyan-400 drop-shadow-md mb-2">
                {((latestRun.accuracyAfter ?? 0) * 100).toFixed(1)}%
              </div>
              <div className="text-xs font-bold text-white/30 uppercase tracking-widest">Latest run</div>
            </GlassCard>
          </motion.div>
          <motion.div variants={staggerChild as any}>
            <GlassCard className="hover:scale-[1.02] transition-transform">
              <div className="text-[10px] text-white/40 font-bold uppercase tracking-widest mb-4">Improvement</div>
              ...
              <div className="text-xs font-bold text-white/30 uppercase tracking-widest">vs. pre-training</div>
            </GlassCard>
          </motion.div>
          <motion.div variants={staggerChild as any}>
            <GlassCard className="hover:scale-[1.02] transition-transform">
              <div className="text-[10px] text-white/40 font-bold uppercase tracking-widest mb-4">Data Points Used</div>
              <div className="flex items-center gap-3 mb-2">
              ...
              <div className="text-xs font-bold text-white/30 uppercase tracking-widest">experimental results</div>
            </GlassCard>
          </motion.div>
        </motion.div>
      )}
        ...
        {chartData.length > 0 && (
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.3 }} className="lg:col-span-2">
            <GlassCard className="h-full">
              ...
              <h2 className="text-2xl font-serif font-bold text-white mb-8 flex items-center gap-3 relative z-10">
                <TrendingUp className="w-6 h-6 text-cyan-400" />
                Accuracy Trend
              </h2>
              <div className="bg-black/40 p-6 rounded-3xl border border-white/5 relative z-10 h-[300px]">
          ...
          <GlassCard className="flex-1 flex flex-col">
            ...
            {isLoading ? (
              <div className="space-y-4">
                {[1, 2, 3].map((i) => <Skeleton key={i} className="h-28 w-full bg-white/5 rounded-[2rem]" />)}
              </div>
            ) : !runs || runs.length === 0 ? (
              <div className="flex-1 flex flex-col items-center justify-center p-8 text-center">
                <Settings2 className="w-12 h-12 text-white/20 mb-4" />
                <p className="text-white/40 font-medium">No Retraining Runs Yet</p>
              </div>
            ) : (
              <div className="space-y-4 overflow-y-auto pr-2 custom-scrollbar flex-1 max-h-[350px]">
                {runs.map((run: RetrainingRun, i: number) => (
                  <div key={run.id} className="p-5 rounded-2xl bg-black/40 border border-white/5 hover:bg-white/5 transition-colors group">
                    <div className="flex items-start justify-between gap-4 mb-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <div className="font-bold text-white text-lg">Run #{runs.length - i}</div>
                    ...
                    <div className="flex items-center justify-between bg-[#050505] p-3 rounded-xl border border-white/5">
                      <div className="font-mono text-sm">
                        <span className="text-white/40">{((run.accuracyBefore ?? 0) * 100).toFixed(1)}%</span>
                        <span className="text-white/20 mx-2">→</span>
                        <span className="text-cyan-400 font-bold">{((run.accuracyAfter ?? 0) * 100).toFixed(1)}%</span>
                      </div>
                      <div className="text-[10px] text-white/30 font-mono font-bold uppercase tracking-widest">
