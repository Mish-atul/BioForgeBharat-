...
function DeltaIndicator({ predicted, measured, label }: { predicted: number; measured: number; label: string }) {
  ...
  const color = isMatch ? "text-white/50" : isUp ? "text-emerald-400" : "text-rose-400";
  const Icon = isMatch ? Minus : isUp ? TrendingUp : TrendingDown;
  const sign = isUp ? "+" : !isMatch ? "-" : "";

  return (
    <div className="flex flex-col justify-center items-center p-6 rounded-3xl bg-black/40 border border-white/5 shadow-inner">
      <div className="text-[10px] text-white/40 uppercase tracking-widest font-bold mb-3">{label}</div>
      <div className="flex items-center gap-4">
        <div className="text-4xl font-mono font-black text-white drop-shadow-md">{(measured * 100).toFixed(1)}%</div>
        <div className={cn("flex flex-col items-center justify-center px-3 py-1.5 rounded-xl bg-white/5 border border-white/10 shadow-inner", color)}>
          <Icon className="w-4 h-4 mb-1" />
          <span className="text-[10px] font-mono font-bold">{sign}{pct}%</span>
        </div>
      </div>
      <div className="text-xs text-white/30 font-mono mt-3">
        Predicted: <span className="font-bold text-white/50">{(predicted * 100).toFixed(1)}%</span>
      </div>
    </div>
  );
}
    ...
    return (
      <div className="space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {[1, 2, 3].map((i) => <Skeleton key={i} className="h-40 bg-white/5 rounded-[2.5rem]" />)}
        </div>
        <Skeleton className="h-80 bg-white/5 rounded-[2.5rem]" />
      </div>
    );
  }
    ...
    <motion.div initial="hidden" animate="visible" variants={staggerContainer as any} className="space-y-8 pb-20">
      ...
      <motion.div variants={fadeUp as any} custom={0} className="flex items-start gap-6">
        ...
        <div className="flex-1">
          ...
          <h1 className="text-4xl md:text-5xl font-serif font-black tracking-tight text-white drop-shadow-lg mb-2">
        ...
        <div className="text-right text-xs text-white/50 font-medium">
          <div className="flex items-center justify-end gap-2 mb-2 bg-white/5 px-3 py-1.5 rounded-full border border-white/10">
            <User className="w-3 h-3 text-orange-400" />
            <span className="text-white font-bold">{experiment.researcherName}</span>
          </div>
      ...
      <motion.div variants={staggerContainer as any} className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <motion.div variants={staggerChild as any}>
          <GlassCard className="h-full">
            <div className="flex flex-col justify-center items-center p-6 rounded-3xl bg-black/40 border border-white/5 shadow-inner h-full">
              <div className="text-[10px] text-white/40 uppercase tracking-widest font-bold mb-3">Measured Yield</div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <motion.div variants={fadeUp as any} custom={4} className="space-y-8 flex flex-col">
          <GlassCard className="flex-1">
            <div className="flex items-center justify-between mb-8 relative z-10">
              ...
              <Button
                ...
                className="gap-2 bg-gradient-to-r from-fuchsia-600 to-rose-500 text-white rounded-full px-6 py-5 font-bold shadow-[0_0_20px_rgba(217,70,239,0.3)] hover:shadow-[0_0_30px_rgba(217,70,239,0.5)] transition-all"
              >
            ...
            <div className="relative z-10 flex-1">
              {experiment.discrepancyHypothesis ? (
                <div className="p-6 rounded-3xl border border-fuchsia-500/30 bg-fuchsia-500/10 shadow-inner">
                  ...
                  <p className="text-base leading-relaxed text-white/90 font-medium">{experiment.discrepancyHypothesis}</p>
                </div>
              ) : (
                <div className="flex flex-col items-center justify-center p-12 text-center h-full bg-black/40 rounded-3xl border border-white/5">
          ...
          {experiment.notes && (
            <GlassCard>
              <h2 className="text-xl font-serif font-bold text-white mb-6 relative z-10 flex items-center gap-3">
                <FileText className="w-5 h-5 text-white/50" />
                Researcher Notes
              </h2>
              <div className="relative z-10 bg-black/40 border border-white/5 rounded-3xl p-6 shadow-inner">
                <p className="text-base leading-relaxed text-white/80 font-medium">{experiment.notes}</p>
              </div>
            </GlassCard>
          )}
        </motion.div>
