...
function MetricBar({ label, value, color = "bg-primary" }: { label: string; value: number; color?: string }) {
  return (
    <div className="space-y-2">
      <div className="flex justify-between text-[10px] font-bold uppercase tracking-widest text-slate-900/50">
        <span>{label}</span>
        <span className="font-mono text-slate-900/90">{(value * 100).toFixed(1)}%</span>
      </div>
      <div className="h-1.5 bg-white/50 rounded-full overflow-hidden border border-white/40">
        <div
...
function SortIcon({ active, dir }: { active: boolean; dir: SortDir }) {
  if (!active) return <ArrowUpDown className="w-3 h-3 text-slate-900/20" />;
  return dir === "asc" ? <ArrowUp className="w-3 h-3 text-slate-900" /> : <ArrowDown className="w-3 h-3 text-slate-900" />;
}
...
function AgentTrace({ isBio }: { isBio: boolean }) {
  ...
  return (
    <GlassCard className="p-1 rounded-[2rem]">
      <div className="p-4 md:p-6 bg-white/60 backdrop-blur-3xl rounded-[calc(2rem-0.375rem)] border border-white/40">
        <div className="flex items-center gap-3 text-sm text-slate-900 font-bold uppercase tracking-widest mb-6">
          <BrainCircuit className="w-5 h-5 text-fuchsia-400" />
          Virtual Research Swarm
        </div>
        <div className="grid grid-cols-2 md:grid-cols-6 gap-3">
          {agents.map(([name, detail]) => (
            <div key={name} className="rounded-2xl border border-white/40 bg-white/40 p-4 shadow-inner flex flex-col items-center text-center">
              <CheckCircle2 className="w-5 h-5 text-emerald-400 mb-2 drop-shadow-[0_0_8px_rgba(52,211,153,0.8)]" />
              <div className="text-sm font-bold text-slate-900 mb-1">{name}</div>
              <div className="text-[9px] text-slate-900/40 leading-relaxed uppercase tracking-widest font-bold">{detail}</div>
            </div>
          ))}
        </div>
      </div>
  ...
  if (rxnLoading) {
    return (
      <div className="space-y-8">
        ...
        <Skeleton className="h-48 w-full bg-white/30 rounded-[2.5rem]" />
        <Skeleton className="h-96 w-full bg-white/30 rounded-[2.5rem]" />
      </div>
    );
  }
  ...
  if (!reaction) {
    return (
      <div className="text-center py-32">
        <Beaker className="w-16 h-16 mx-auto mb-6 text-slate-900/20" />
        <h2 className="text-3xl font-serif font-bold text-slate-900 mb-2">Reaction Disconnected</h2>
        <p className="text-slate-900/50 mb-8">This chemical pathway no longer exists in the registry.</p>
        <Link href="/reactions">
          <Button className="bg-white/10 text-slate-900 hover:bg-white/20 rounded-full px-8 py-6 font-bold border border-white/60">Return to Library</Button>
        </Link>
      </div>
    );
  }
  ...
  return (
    <motion.div initial="hidden" animate="visible" variants={{ visible: { transition: { staggerChildren: 0.1 } } } as any} className="space-y-10 pb-20 max-w-[1400px] mx-auto">
        ...
        <div className="flex items-start gap-6">
          ...
          <div>
            <div className="flex items-center gap-3 mb-3">
              <div className="w-8 h-8 rounded-full bg-white/30 border border-white/50 flex items-center justify-center shadow-inner">
              ...
              <Badge variant="outline" className="text-[10px] font-bold uppercase tracking-widest px-3 py-1 rounded-full bg-white/30 text-slate-900/70 border-white/50">
                {reaction.type}
              </Badge>
            </div>
            <h1 className="text-4xl md:text-5xl font-serif font-black tracking-tight text-slate-900 drop-shadow-lg">{reaction.name}</h1>
          </div>
        </div>
        <div className="flex items-center gap-3">
          <Button
            variant="outline"
            className="rounded-full bg-white/30 border-white/50 hover:bg-white/10 text-slate-900 font-bold h-12 px-6 shadow-lg"
            onClick={() => {
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 relative z-10">
            <div className="space-y-6">
              <div className="bg-white/40 p-6 rounded-3xl border border-white/40 shadow-inner">
                <div className="text-[10px] text-slate-900/40 uppercase tracking-widest font-bold mb-3">Reaction Equation</div>
                <div className="font-mono text-xl md:text-2xl font-black text-cyan-400 drop-shadow-md">
                  {reaction.equation}
                </div>
              </div>
              <div className="bg-white/40 p-6 rounded-3xl border border-white/40 shadow-inner">
                <div className="text-[10px] text-slate-900/40 uppercase tracking-widest font-bold mb-2">Target Product</div>
                <div className="text-lg font-bold text-slate-900">{reaction.targetProduct}</div>
              </div>
            </div>
            <div className="space-y-6">
              <div className="bg-white/40 p-6 rounded-3xl border border-white/40 shadow-inner">
                <div className="text-[10px] text-slate-900/40 uppercase tracking-widest font-bold mb-3">Thermodynamic Conditions</div>
                <div className="font-mono text-base font-bold text-orange-400 drop-shadow-md">
                  {reaction.conditions}
                </div>
              </div>
              <div className="bg-white/40 p-6 rounded-3xl border border-white/40 shadow-inner h-full">
                <div className="text-[10px] text-slate-900/40 uppercase tracking-widest font-bold mb-2">Notes</div>
                <div className="text-sm font-medium text-slate-900/70 leading-relaxed">{reaction.description}</div>
              </div>
            </div>
          </div>
        </GlassCard>
      ...
      <motion.div variants={fadeUp as any} custom={3} className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <GlassCard>
          <div className="flex flex-col justify-between h-full relative z-10">
            <div className="flex items-center gap-3 text-lg font-serif font-bold text-slate-900 mb-6">
            ...
            <div className="space-y-4 bg-white/40 p-6 rounded-3xl border border-white/40 shadow-inner">
              <div className="flex gap-3">
                <Input
                  ...
                  className="h-14 rounded-2xl bg-white/30 border-white/50 font-mono text-slate-900 focus-visible:ring-blue-500 text-lg px-4"
                />
                <Button
                  onClick={handleSearch}
                  disabled={searchCandidates.isPending || searchQuery.trim().length === 0}
                  className="h-14 px-8 rounded-2xl bg-blue-600 hover:bg-blue-500 text-slate-900 font-bold shadow-[0_0_20px_rgba(59,130,246,0.3)] hover:shadow-[0_0_30px_rgba(59,130,246,0.5)] transition-all"
                >
        ...
        <GlassCard>
          ...
          <div className="flex flex-col justify-between h-full relative z-10">
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-3 text-lg font-serif font-bold text-slate-900">
            ...
            <div className="bg-white/40 p-6 rounded-3xl border border-white/40 shadow-inner text-center flex flex-col items-center justify-center h-full min-h-[120px]">
              <Button
                onClick={handleGenerate}
                disabled={generating || generateCandidates.isPending}
                className="w-full max-w-sm h-14 rounded-full bg-gradient-to-r from-fuchsia-600 to-orange-500 text-slate-900 font-bold text-lg shadow-[0_0_20px_rgba(217,70,239,0.3)] hover:shadow-[0_0_40px_rgba(249,115,22,0.5)] hover:-translate-y-1 transition-all"
              >
      ...
      <motion.div variants={fadeUp as any} custom={4} className="space-y-6">
        ...
        <div className="flex items-center justify-between bg-white/40 p-4 rounded-3xl border border-white/40 shadow-inner flex-wrap gap-4">
          <div className="flex items-center gap-4">
            <h2 className="text-2xl font-serif font-bold text-slate-900 ml-2">Candidates</h2>
            {candidates && (
              <Badge className="bg-white/10 text-slate-900 hover:bg-white/20 border border-white/60 font-mono">
          ...
          <div className="flex items-center gap-4 flex-wrap">
            <ToggleGroup type="single" value={viewMode} onValueChange={(v) => v && setViewMode(v as "cards" | "table")} className="bg-white/30 p-1 rounded-xl border border-white/50">
              <ToggleGroupItem value="cards" className="rounded-lg data-[state=on]:bg-white/10 data-[state=on]:text-slate-900 text-slate-900/50 px-4">
                <LayoutGrid className="w-4 h-4 mr-2" /> <span className="text-xs font-bold">Cards</span>
              </ToggleGroupItem>
              <ToggleGroupItem value="table" className="rounded-lg data-[state=on]:bg-white/10 data-[state=on]:text-slate-900 text-slate-900/50 px-4">
                <Rows3 className="w-4 h-4 mr-2" /> <span className="text-xs font-bold">Table</span>
              </ToggleGroupItem>
            </ToggleGroup>

            <Select value={sourceFilter} onValueChange={(v) => setSourceFilter(v as SourceFilter)}>
              <SelectTrigger className="w-40 h-10 rounded-xl bg-white/30 border-white/50 text-slate-900 font-medium focus:ring-fuchsia-500">
                <SelectValue />
              </SelectTrigger>
              <SelectContent className="bg-[#110F1A] border-white/50 text-slate-900 rounded-xl">
            ...
            {viewMode === "cards" && (
              <div className="flex items-center gap-2 bg-white/30 p-1 rounded-xl border border-white/50">
                <Select value={sortBy} onValueChange={(v) => setSortBy(v as SortBy)}>
                  <SelectTrigger className="w-40 h-8 rounded-lg bg-transparent border-0 text-slate-900 font-medium focus:ring-0 shadow-none">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent className="bg-[#110F1A] border-white/50 text-slate-900 rounded-xl">
                ...
                <Button variant="ghost" size="icon" className="h-8 w-8 rounded-lg text-slate-900 hover:bg-white/10" onClick={() => setSortDir((d) => (d === "asc" ? "desc" : "asc"))}>
        ...
        {candLoading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {[1, 2, 3, 4].map((i) => <Skeleton key={i} className="h-48 w-full bg-white/30 rounded-[2.5rem]" />)}
          </div>
        ) : !candidates || candidates.length === 0 ? (
          <GlassCard>
            <div className="py-20 text-center flex flex-col items-center">
              <div className="w-20 h-20 rounded-full bg-white/30 border border-white/50 flex items-center justify-center mb-6">
                <Sparkles className="w-10 h-10 text-slate-900/20" />
              </div>
              <h3 className="text-2xl font-serif font-bold text-slate-900 mb-3">Registry Empty</h3>
              <p className="text-slate-900/50 text-lg max-w-md mx-auto">
                {sourceFilter === "all"
            ...
            {candidates.map((candidate, i) => (
              <motion.div key={candidate.id} variants={staggerChild as any}>
                <Link href={`/candidates/${candidate.id}`}>
                  <div className="p-1.5 rounded-[2rem] bg-white/[0.04] border border-white/50 shadow-lg group hover:bg-gradient-to-br hover:from-fuchsia-500/20 hover:to-orange-500/20 transition-all duration-500 cursor-pointer hover:-translate-y-1">
                    <div className="h-full w-full rounded-[calc(2rem-0.375rem)] bg-white/70 border border-white/40 p-6 flex flex-col relative overflow-hidden">
                      <div className="flex items-start justify-between gap-4 mb-6">
                        <div className="flex items-center gap-4">
                          <div className="w-12 h-12 rounded-full bg-white/50 border border-white/50 flex items-center justify-center font-mono font-black text-slate-900/40 group-hover:text-fuchsia-400 group-hover:border-fuchsia-500/40 transition-colors shadow-inner text-xl">
                            {candidate.rank ?? i + 1}
                          </div>
                          <div>
                            <div className="flex items-center gap-3 mb-1">
                              <h4 className="text-xl font-bold text-slate-900 group-hover:text-slate-900 transition-colors">{candidate.name}</h4>
                            ...
                            <div className="font-mono text-sm text-slate-900/50 tracking-tight font-bold">{candidate.formula}</div>
                          </div>
                        </div>
                        <div className="w-10 h-10 rounded-full bg-white/30 border border-white/50 flex items-center justify-center group-hover:bg-white/10 transition-colors flex-shrink-0">
                          <ArrowRight className="w-5 h-5 text-slate-900/50 group-hover:text-slate-900 group-hover:translate-x-1 transition-all" />
                        </div>
                      </div>

                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 bg-white/40 p-4 rounded-2xl border border-white/40 shadow-inner">
          ...
          <GlassCard className="p-1 rounded-[2rem]">
            <div className="rounded-[calc(2rem-0.375rem)] bg-white/60 overflow-hidden border border-white/40">
              <Table>
                <TableHeader>
                  <TableRow className="border-white/50 bg-white/40 hover:bg-white/40">
                    <TableHead className="w-16 py-4">
                      <button type="button" onClick={() => toggleSort("rank")} className="flex items-center gap-2 text-[10px] uppercase font-bold tracking-widest text-slate-900/60 hover:text-slate-900 transition-colors">
                        Rank <SortIcon active={sortBy === "rank"} dir={sortDir} />
                      </button>
                    </TableHead>
                    <TableHead className="py-4">
                      <span className="text-[10px] uppercase font-bold tracking-widest text-slate-900/60">Candidate</span>
                    </TableHead>
                    <TableHead className="py-4">
                      <span className="text-[10px] uppercase font-bold tracking-widest text-slate-900/60">Source</span>
                    </TableHead>
                    {sortableMetricColumns.map((col) => (
                      <TableHead key={col.key} className="text-right py-4">
                        <button type="button" onClick={() => toggleSort(col.key)} className="inline-flex items-center gap-2 text-[10px] uppercase font-bold tracking-widest text-slate-900/60 hover:text-slate-900 transition-colors w-full justify-end">
                ...
                <TableBody>
                  {candidates.map((c, i) => (
                    <TableRow key={c.id} className="border-white/40 hover:bg-white/30 cursor-pointer group transition-colors">
                      <TableCell className="font-mono text-slate-900/40 font-bold text-center">{c.rank ?? i + 1}</TableCell>
                      <TableCell>
                        <Link href={`/candidates/${c.id}`} className="block w-full h-full py-2">
                          <div className="font-bold text-slate-900 group-hover:text-fuchsia-300 transition-colors mb-1">{c.name}</div>
                          <div className="font-mono text-xs text-slate-900/40">{c.formula}</div>
                        </Link>
                      </TableCell>
                      <TableCell>
                      ...
                      {sortableMetricColumns.map((col) => {
                        ...
                        return (
                          <TableCell key={col.key} className={cn("text-right font-mono text-sm", isBest ? "text-fuchsia-400 font-black drop-shadow-md" : "text-slate-900/70 font-medium")}>
                      ...
                      <TableCell>
                        <Link href={`/candidates/${c.id}`}>
                          <div className="w-8 h-8 rounded-full bg-white/30 flex items-center justify-center group-hover:bg-fuchsia-500/20 transition-colors ml-auto">
                            <ArrowRight className="w-4 h-4 text-slate-900/40 group-hover:text-fuchsia-400 group-hover:translate-x-0.5 transition-all" />
                          </div>
                        </Link>
                      </TableCell>
                    </TableRow>
