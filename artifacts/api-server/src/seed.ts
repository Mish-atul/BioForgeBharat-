...
\n  const reactionId = rxnEtj.id;
  ...
  const insertedEtjCandidates = await db.insert(candidatesTable).values(
    etjCandidates.map((c, i) => ({ ...enrichCandidate(c, "chemical-catalysis", i, "Sustainable Aviation Fuel (C₈–C₁₆)"), reactionId }))
  ).returning();
  ...
  const insertedCO2Candidates = await db.insert(candidatesTable).values(
    co2Candidates.map((c, i) => ({ ...enrichCandidate(c, "chemical-catalysis", i, "Sustainable Aviation Fuel (C₈–C₁₆)"), reactionId: co2RxnId }))
  ).returning();
  ...
  await db.insert(candidatesTable).values(
    fermCandidates.map((c, i) => ({ ...enrichCandidate(c, "synthetic-biology", i, "Bioethanol (>95% purity)"), reactionId: fermRxnId }))
  ).returning();
